...
function VideoFrame({
  ...
  return (
    <div className="relative w-full overflow-hidden border border-secondary/30 bg-black shadow-[0_0_0_1px_rgba(197,160,89,0.10)]" data-testid={testId}>
    ...
    <div className="min-h-screen flex flex-col bg-primary text-primary-foreground">
      ...
      <main className="flex-grow pt-32 pb-24">
        <div className="max-w-7xl mx-auto px-6">
          ...
          <section data-testid="section-videos-latest">
            <div className="flex items-center justify-between gap-6" data-testid="row-videos-latest-header">
              <h2 className="font-display text-3xl md:text-4xl tracking-tight" data-testid="text-videos-latest-title">
                LATEST BROADCASTS
              </h2>
            </div>

            <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-6" data-testid="grid-videos-latest">
              {broadcasts.map((v, idx) => (
                <article
                  key={v.id}
                  ...
                  data-testid={`card-videos-latest-${idx}`}
                >
                  <VideoFrame id={v.id} title={v.title} testId={`video-latest-${idx}`} />

                  <div className="p-6" data-testid={`section-videos-latest-meta-${idx}`}>
                    <div
                      className="text-xs font-bold uppercase tracking-widest text-secondary"
                      data-testid={`text-videos-latest-badge-${idx}`}
                    >
                      {v.date}
                    </div>
                    <h3
                      className="mt-3 font-display text-xl text-primary-foreground"
                      data-testid={`text-videos-latest-title-${idx}`}
                    >
                      {v.title}
                    </h3>
                    <p
                      className="mt-3 text-sm leading-relaxed text-primary-foreground/75"
                      data-testid={`text-videos-latest-context-${idx}`}
                    >
                      {v.context}
                    </p>
                  </div>
